...
def calculate_return(stock_symbol):
    ...
    cash = initial_cash
    shares = 0
    stop_loss_percentage = 0.05  # 3% stop loss
    ...
    for i in range(3, len(stock_data)):
        ...
        cumulative_percentage_change = (
            (stock_data['Close'][i-3] - stock_data['Open'][i-3]) / stock_data['Open'][i-3] +
            (stock_data['Close'][i-2] - stock_data['Open'][i-2]) / stock_data['Open'][i-2] +
            (stock_data['Close'][i-1] - stock_data['Open'][i-1]) / stock_data['Open'][i-1]
        )
        ...
        if cumulative_percentage_change < 0:
            ...
            if sell_price == 0:
                ...
                elif stock_data['Low'][i] < buy_price * (1 - stop_loss_percentage):
                    sell_price = buy_price * (1 - stop_loss_percentage)
                # Sell on the next Monday
                else:
                    next_monday_index = i + (7 - i % 7)
                    sell_price = stock_data['Open'][next_monday_index]
