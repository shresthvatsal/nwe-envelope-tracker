...
def calculate_nwe(stock_data, h=8, mult=3):
    n = len(stock_data)

    if n == 0:
        return pd.DataFrame({'Upper': [], 'Lower': []}, index=[])

    # Gaussian window
    gauss = lambda x: np.exp(-(x ** 2) / (h ** 2 * 2))

    # Compute coefficients
    coefs = np.array([gauss(i) for i in range(n)])
    den = coefs.sum()

    # Calculate NWE
    out = np.sum(stock_data['Close'] * coefs) / den
    mae = np.mean(np.abs(stock_data['Close'] - out)) * mult

    upper = out + mae
    lower = out - mae

    return pd.DataFrame({'Upper': upper, 'Lower': lower}, index=stock_data.index[-1:])
...
def main():
    ...
    for stock_symbol in stocks:
        try:
            ...
            stock_data = yf.download(stock_symbol, start="2020-01-01", end="2023-11-28")

            if stock_data.empty:
                pass
                continue

            # Calculate Nadaraya-Watson Envelope for the last available trading day
            nwe_data = calculate_nwe(stock_data)

            # Check if the last closing price is below the lower envelope or above the upper envelope
            last_close = stock_data['Close'].iloc[-1]
            if last_close < nwe_data['Lower'].iloc[0]:
                print(f"{stock_symbol}: Price below Nadaraya-Watson Lower Envelope")
            elif last_close > nwe_data['Upper'].iloc[0]:
                print(f"{stock_symbol}: Price above Nadaraya-Watson Upper Envelope")
